...
let rec cmp_stmt (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  | Ret (Some(exp)) -> let ty,op,stream = cmp_exp c exp in c,stream>@[T(Ret (ty,Some(op)))]
...
let cmp_global_ctxt (c:Ctxt.t) (p:Ast.prog) : Ctxt.t =
  List.fold_left (fun c decl -> match decl with 
  | Gvdecl {elt={name;init={elt=exp}}} ->
    let ty = match exp with 
      | CNull t -> Ptr(cmp_rty t)
      | CBool b -> Ptr(I1)
      | CInt  i -> Ptr(I64)
      | CStr  s -> Ptr(I8)
      | CArr  (ty, list) -> Ptr(Struct ([I64; Array (0,cmp_ty ty)]))
      | _ -> failwith "bad global decl"
    in
...
let cmp_fdecl (c:Ctxt.t) (f:Ast.fdecl node) : Ll.fdecl * (Ll.gid * Ll.gdecl) list =
  ...
  let f_ty= Ctxt.lookup_function_option fname c in
  let arg_names=(List.map snd args) in
  let f_param = List.map gensym arg_names in 
  let stream,c = List.fold_left2 (fun (stream,c) (arg_ty,arg_name) uid ->
    let aloc_uid=gensym "var" in 
    stream>::I (aloc_uid,Alloca(cmp_ty arg_ty))
    >::I (gensym "store",Store(cmp_ty arg_ty, Id uid, Id aloc_uid)),
    Ctxt.add c arg_name (cmp_ty arg_ty, Id(aloc_uid))) 
    ([],c) args f_param in
  let c,stream = cmp_block c (cmp_ret_ty frtyp) body in
  let f_cfg,ll_list=cfg_of_stream stream in
  match f_ty with
  |Some(Ptr(Fun(args,ret)),_)->
  {f_ty=(args,ret);f_param;f_cfg}, ll_list
  |_->failwith "bad func"
...
let rec cmp_gexp c (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  match e.elt with
  | CNull t -> ((cmp_rty t, GNull),[])
  | CBool b -> ((I1, GInt (if b then 1L else 0L)),[])
  | CInt  i -> ((I64, GInt i),[])
  | CStr  s -> 
    let gid=gensym "global_string" in 
    let gid_ty = Array (1+String.length s, I8) in
    ((Ptr(I8), GBitcast (Ptr(gid_ty),GGid(gid), Ptr(I8))), [(gid,(gid_ty,GString s))])
  | CArr (ty,list) -> 
    let gid=gensym "global_arr" in 
    let gid_ty = Struct([I64;Array (List.length list, cmp_ty ty)]) in
    let final_ty = Ptr(Struct ([I64; Array (0,cmp_ty ty)])) in
    let (elt_list,gdec_list) = List.fold_right (fun exp (elt_list,gdec_list)->
      let (gdec,list) = cmp_gexp c exp in (gdec::elt_list,list@gdec_list)) list ([],[]) in
    ((final_ty, GBitcast (Ptr(gid_ty),GGid(gid),final_ty)),[(gid,(gid_ty,GArray elt_list))]@gdec_list)
  | _ -> failwith "not implemented cmp_gexp"
